app
  ...
  .directive('block', [
    ...
    'general',
    'data',
    function ($stateParams, states, general, data) {
      return {
        resrict: 'E',
        scope: {
          type: '@',
        ...
        link: function ($scope, element, attr) {
          ...
          $scope.openVariable = function (variable) {
            ...
            if (
              $scope.data.id == 'variables' ||
              $scope.parent.id == 'variables'
            ) {
              objectKeys = ['doc', 'items', 'sets', 'items'];

              dataKeys = ['name', 'id', 'id', 'name'];
              ...
              $scope.data.id == 'functions' ||
              $scope.parent.id == 'functions'
            ) {
              // console.log("parent", $scope.data);

              var inputoutput =
                ($scope.data.name.text == 'returns'
                  ? 'output'
                  : $scope.data.name.text.slice(
                      0,
                      $scope.data.name.text.length - 1
                    )) + '.items';

              objectKeys = ['doc', 'items', 'sets', 'items', inputoutput];

              dataKeys = ['name', 'id', 'id', 'name', 'name'];
            ...
            states.go('variable', {
              ...
              variableData: { misc: $scope.misc, data: variableData },
            });
          };
  ...
  .directive('footer', function () {
    return {
      ...
      link: function ($scope, element, attr) {
        ...
        $(element).html(year + ' methods with class');
      },
    };
  })
